...
export function V2ProductDetail({product}:{product:StoreProduct}){
  ...
  return <main className="v2Pdp">
    <section className="v2PdpLayout">
      <div className="v2PdpGallery"><div className="v2PdpPrimary"><img src={product.image} alt={product.name}/></div><div className="v2PdpSecondary"><img src={product.hoverImage} alt={`${product.name} alternate view`}/><div className="v2FabricPanel"><span>{isPermanentTen?"THE OFFER":"MATERIAL"}</span><strong>{isPermanentTen?"10 ties · 1 collection":product.fabric}</strong><p>{isPermanentTen?"$50 per tie":product.badges.join(" · ")||"Permanent house specification"}</p></div></div></div>
