...
export function CapsuleBuilder(){
  ...
  return <section className="capsuleBuilder">
    ...
    <div className="capsuleResult">
      <div className="capsuleSuit" style={{background:suitPalette.find(x=>x[0]===suit)?.[1]}}><span>SELECTED TAILORING</span><strong>{suit}</strong><small>Pairing reference only</small></div>
      <div className="capsuleRules"><div><span>SHIRT FAMILIES</span><b>{matches.shirts.join(" / ")}</b></div><div><span>PERMANENT 10 TIES</span><b>{matches.ties.join(" / ")}</b></div><div><span>LEATHER FAMILY</span><b>{matches.shoes.join(" / ")}</b></div></div>
    </div>
    <div className="capsuleProducts"><Link href={`/products/${permanentTenBundle.slug}`}><img src={permanentTenBundle.image} alt={permanentTenBundle.name}/><span>{permanentTenBundle.name}</span><b>$500 · 10 ties</b></Link></div>
  </section>
}
